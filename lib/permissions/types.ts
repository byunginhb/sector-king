 ...
 *    벗겨지고 네트워크 탭에 원본이 그대로 남는다. `params.hiddenRows` 는
 *    "서버가 상위 몇 개를 지우고 보낼지"를 정하는 값이지 CSS 값이 아니다.
 ...
 *
 * - `open`    게이트 없음 (기본)
 * - `partial` **상위 N개를 가리고** 그 아래를 실값으로 준다. 리스트·표처럼
 *             **쪼갤 단위가 있는 기능에만** 쓴다.
 * - `hidden`  값을 전량 제외. 진입점 자체를 렌더하지 않음
 *
 * `partial` 이 상위를 가리는 이유: 순위표에서 궁금한 것은 1·2·3위이고 그게
 * 곧 상품이다. 하위부터 가리면 사용자는 아쉬울 게 없고, 상위를 가리면 표
 * 구조와 순위 번호는 남아 "무엇을 놓치고 있는지"가 보인다.
 *
 ...
 * 게이트 파라미터.
 *
 * `hiddenRows` 는 서버가 **상위에서 지울** 항목 수다(§원칙 1).
 * 미지정이면 `PARTIAL_DEFAULT_HIDDEN_ROWS`(3건)가 적용된다.
 */
export type GateParams = {
  /** 상위 N개를 가리고 그 아래를 노출. `partial` 에서만 의미가 있다. */
  hiddenRows?: number
